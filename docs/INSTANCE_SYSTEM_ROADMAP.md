...
> **Status:** Active development — Wiki import command built, 83 hunts validated via dry run
...
- [x] Wiki import pipeline: Python scraper (514 hunts from tibiawiki.com.br), management UI with auto-classification + server monster validation, direct Lua export (2026-04-06)
- [x] `/hunt catalog` headless fix — survives client disconnect (2026-04-06)
- [x] Curate hunts via management UI — 92 hunts marked for import, exported to Lua (2026-04-07)
- [x] Build `/hunt wikimport` Lua command — reads WIKI_HUNTS, flood-fills from seedPos, merges duplicates, validates, generates files (2026-04-07)
- [x] Monster name normalization — disambiguation suffixes + case-insensitive matching (2026-04-07)
- [ ] Run `/hunt wikimport` (non-dry) to generate 83 hunt registration files
- [ ] Register 10+ hunts via wiki import pipeline
...
### Wiki Import Pipeline (Tools)

| File | Purpose |
|------|---------|
| `tools/scrape_wiki_hunts.py` | Python scraper — fetches 514 hunts from tibiawiki.com.br API |
| `tools/hunt_manager.py` | Local HTTP server — serves UI + API for save/export |
| `tools/hunt_manager.html` | Management UI — auto-classify, monster match, bulk curation |
| `tools/HUNT_MANAGER_USAGE.md` | Full usage documentation for the pipeline |
| `data/hunt_wiki_raw.json` | Scraped wiki data (generated by scraper) |
| `data/hunt_wiki_state.json` | Curation decisions (generated by UI) |
| `data/libs/functions/wiki_hunts_import.lua` | Lua import table (generated by UI export) |
